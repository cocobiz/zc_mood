 ...
 * 整个抽象层主要用来执行select语句，主要的API：
 * queryFactory::Execute($sql, $limit, $isCache, $cacheTime)
 * queryFactory::ExecuteRandomMulti($sql, $limit, $isCache, $cacheTime)
 * queryFactory::perform($table, $tablDta, $performType('insert'), $performFilter('where'))
 * 
 * 上面的方法返回的都是 queryFactoryResult 对象
 * @package classes
...
class queryFactory extends base {
  ...
  function query($query, $link) {
      ...
      $this->total_queries++;
      if( isset($queryCache) && $queryCache->inCache($query) ) {				// if the select sql has execuded, then return the query result.
  ...
  function Execute($zf_sql, $zf_limit = false, $zf_cache = false, $zf_cachetime=0) {
    ...
    $this->zf_sql = $zf_sql;
    if ( $zf_cache AND $zc_cache->sql_cache_exists($zf_sql, $zf_cachetime) ) {					//  if using cache and the query result has stored, get serialize result_array
    ...
    } elseif ($zf_cache) {																		// if using cahce but the query result has not stored. cache it!
      ...
      $this->count_queries++;
      return($obj);
    } else {																							// no cache, directly query sql from database.
      $time_start = explode(' ', microtime());
      $obj = new queryFactoryResult;
      if (!$this->db_connected)
      {
  ...
  * @ $tableData: array(array('fieldName'=>'id', 'type'=>'integer', 'value'=>0), array('fieldName'=>'categories_name', 'type'=>'string', 'value'=>'dvd'))
  * 
  * */
